...
```{r}
model3 = lm(points~countrycat+year+taster_name+log(price),data=train)
summary(model3)
```

```{r}
model4 = lm(points~countrycat+taster_name+year+log(price)+(countrycat+taster_name+year+log(price))^2,data=train)
summary(model4)
```

```{r}
model0 = lm(points~1,data=train)
model5 = step(model3, scope = list(lower = model0, upper = model4), direction = "both",trace=0)
summary(model5)
```
...
```{r}
# forward selection
model6 = step(model1, scope = list(upper = model4), direction = "forward", trace=0)
summary(model6)
```
...
# stepwise
model0 = lm(points~1,data=train)
model7 = step(model3, scope = list(lower = model0, upper = model4), direction = "both",trace=0)
summary(model7)
```
