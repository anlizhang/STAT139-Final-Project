...
```{r}
model25 = lm(points~countrycat,data=train)
summary(model2)
```

```{r}
model35 = lm(points~taster_name*log(price),data=train)
summary(model3)
```
...
=======
plot(train$taster_name,train$points)
```
...
library(lme4)
library(MuMIn)
mixedmodel = lmer(points~log(price)+ (1+log(price)|taster_name), data=subdata)
coef(mixedmodel)
```
...
```{r}
mixedmodel2 = lmer(points~year + (1+year|taster_name), data=train)
coef(mixedmodel2)
```

```{r}
modelhi = lm(points~log(price)*taster_name,data=train)
summary(modelhi)
```

```{r}
model11 = lm(points~taster_name*log(price),data=subdata)
model10 = lm(points~taster_name+log(price),data=subdata)
...
>>>>>>> edc657f7755430aac6202f57eb634634e60c8f76
